...
(function () {
  ...
  // 合同模板数据仓库：localStorage(管理端上传) → data/templates.json → 内置 seed
  var TEMPLATES = [];
  function loadTemplates() {
    try {
      var saved = JSON.parse(localStorage.getItem('yhhn_admin_templates'));
      if (Array.isArray(saved) && saved.length) {
        TEMPLATES = saved;
        renderTplList();
        window.dispatchEvent(new Event('yhhn:data'));
        return;
      }
    } catch (e) {}
    TEMPLATES = SEED_TEMPLATES.slice();
    renderTplList();
    fetch('data/templates.json').then(function (r) { return r.json(); }).then(function (list) {
      if (Array.isArray(list) && list.length && !localStorage.getItem('yhhn_admin_templates')) {
        TEMPLATES = list;
        renderTplList();
        window.dispatchEvent(new Event('yhhn:data'));
      }
  ...
  function renderTplList() {
    var g0 = document.getElementById('tplG0');
    var g1 = document.getElementById('tplG1');
    var g2 = document.getElementById('tplG2');
    if (!g0 && !g1 && !g2) return;
    [g0, g1, g2].forEach(function (g) { if (g) g.innerHTML = ''; });
    (TEMPLATES || []).forEach(function (tplObj) {
      var group;
      if (tplObj.rus) { group = g2; }
      else if (['农产品购销合同', '粮食购销合同', '鲜活农产品购销合同'].indexOf(tplObj.name) !== -1) { group = g0; }
      else { group = g1; }
      if (!group) return;
      var row = document.createElement('div');
      row.className = 'tpl-row';
      row.innerHTML = '<span class="tpl-name">' + esc(tplObj.name) + '</span>' +
        '<span class="tpl-badge' + (tplObj.rus ? ' rus-badge' : '') + '">' + esc(tplObj.badge || '通用') + '</span>' +
        '<button class="tpl-dl" data-tpl-dl>下载模板</button>';
      ...
      row.addEventListener('click', function (e) {
        ...
        openSheet('合同模板预览', '\
          <div class="tpl-pv-head">📄 ' + esc(tplObj.name) + ' <span class="tpl-badge ' + (tplObj.rus ? 'rus-badge' : '') + '">' + esc(tplObj.badge || '通用') + '</span></div>\
          <div class="dt-desc" style="margin-top:10px">' + esc(tplObj.desc || '') + '</div>\
          <div class="dt-sec-head">主要条款</div>\
          <ul class="outline">' + (tplObj.feat || []).map(function (f) { return '<li>' + esc(f) + '</li>'; }).join('') + '</ul>\
          <div class="dt-sec-head">📝 填写信息，生成合同草稿</div>\
          <div class="fill-fields">' +
            (tplObj.fields || []).map(function (f) {
              return '<div class="fill-row"><div class="fill-label">' + esc(f.label) + '</div>' +
                '<input class="tpl-fill-input" data-fill="' + esc(f.key) + '" placeholder="' + esc(f.ph || '') + '" /></div>';
            }).join('') +
          '</div>\
          <button class="primary-btn tpl-gen-draft" data-tplname="' + esc(tplObj.name) + '" style="width:100%">生成合同草稿</button>\
          <div class="tpl-draft-out" id="tplDraftOut" style="display:none;white-space:pre-wrap"></div>\
          <div class="dt-actions tpl-draft-actions" id="tplDraftActions" style="display:none">\
            <button class="primary-btn tpl-draft-copy" type="button">📋 复制草稿</button>\
            <button class="fav-btn ghost tpl-draft-dl" type="button">⬇ 下载为文件</button>\
          </div>\
  ...
  document.addEventListener('click', function (e) {
    ...
    var acts = document.getElementById('tplDraftActions');
  ...
  // 复制草稿
  document.addEventListener('click', function (e) {
    var cb = e.target.closest('.tpl-draft-copy');
    if (!cb) return;
    var out = document.getElementById('tplDraftOut');
    if (!out || !out.textContent) { toast('请先生成草稿'); return; }
    copyText(out.textContent, function () {
      toast('草稿已复制 ✅');
    });
  });

  // 下载草稿为文件（_.txt，兼容无后端环境）
  document.addEventListener('click', function (e) {
    var dl = e.target.closest('.tpl-draft-dl');
    if (!dl) return;
    var out = document.getElementById('tplDraftOut');
    if (!out || !out.textContent) { toast('请先生成草稿'); return; }
  ...
  function bumpSurvey() {
    var el = document.getElementById('statSurvey');
    if (el) el.textContent = (parseInt(el.textContent || '0', 10)) + 1;
  }
  ...
  eduApply.forEach(function (btn) {
    btn.addEventListener('click', function (e) {
      ...
      openSheet('定制合同审核', '\
        ...
        <div class="rule-tip" style="margin:10px 0 0">💡 您可以同时上传证据与材料，详见法律服务-证据留存</div>\
      ');
    });
  });
  document.addEventListener('click', function (e) {
  ...
  (function () {
    ...
    window.addEventListener('yhhn:data', function () { INDEX = buildIndex(); });
  ...
  (function () {
    var SEED_NEWS = [
      { title: '盛夏普法行 · 青春助农忙', school: '黑龙江大学 法学院', date: '2026-07-20', content: '同学们走进XX村，用方言短剧讲解土地流转与购销合同，现场答疑40余人次。' },
      { title: '农技下乡 · 产销面对面', school: '东北农业大学 农学院', date: '2026-07-15', content: '深入田间指导农资选购，组织农户与电商平台进行产销对接洽谈。' }
    ];
    // 三下乡资讯：localStorage(管理端上传) → data/sxnews.json → 内置 seed
    var NEWS = [];
    function renderNews() {
      var box = document.getElementById('sxNews');
      if (!box) return;
      box.innerHTML = NEWS.map(function (n) {
        return '<div class="sx-card">' +
          '<div class="sx-head">' + esc(n.title) + ' <span class="sx-date">' + esc(n.date) + '</span></div>' +
          '<div class="sx-school">🎓 ' + esc(n.school) + '</div>' +
          '<div class="sx-brief">' + esc(n.content || n.brief || '') + '</div>' +
          '</div>';
      }).join('');
    }
    function loadNews() {
      try {
        var saved = JSON.parse(localStorage.getItem('yhhn_admin_sxnews'));
        if (Array.isArray(saved) && saved.length) {
          NEWS = saved;
          renderNews();
          return;
      ...
      renderNews();
      fetch('data/sxnews.json').then(function (r) { return r.json(); }).then(function (list) {
        if (Array.isArray(list) && list.length && !localStorage.getItem('yhhn_admin_sxnews')) {
          NEWS = list;
          renderNews();
        }
      }).catch(function () {});
    ...
    document.addEventListener('click', function (e) {
      var sign = e.target.closest('#sxSignBtn');
      if (sign) {
        var n = ($('#sxName') || {}).value || '';
        var sch = ($('#sxSchool') || {}).value || '';
        var ph = ($('#sxPhone') || {}).value || '';
        if (!n.trim()) { toast('请填写姓名'); return; }
        if (!sch.trim()) { toast('请填写学校 / 学院'); return; }
        if (!/^1\d{10}$/.test(ph.trim())) { toast('请填写正确的手机号'); return; }
        var dirs = $all('#sxDir .opt-chip.active').map(function (c) { return c.textContent.trim(); });
        toast('同学您好，三下乡报名已收到（' + (dirs.length ? dirs.join('、') : '综合方向') + '）🎉');
        bumpSurvey();
        $('#sxName').value = ''; $('#sxSchool').value = ''; $('#sxPhone').value = '';
        $all('#sxDir .opt-chip').forEach(function (c) { c.classList.remove('active'); });
      }
    });
  })();
